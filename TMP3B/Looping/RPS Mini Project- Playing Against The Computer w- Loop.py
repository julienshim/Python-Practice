...
while (initializing):
    ...
    while player_wins < winning_score and computer_wins < winning_score:
        ...
        if player == 'quit' or player == 'q':
            break
        # not sure why randomint was recommended when choice removes a lot of steps. # noqa
        computer = choice(movesList)

        # impromtu validation
        if player not in validMoves:
            player = False

        if computer not in validMoves:
            computer = False
        ...
        def result(win_res, los_res, winner):
            ...
            print(f'{win_res} beats {los_res}. '
                  f'{winner.capitalize()} earns 1 point!')
    ...
    if play_again == 'n':
        ...
        play_again = input(
            'Sorry, I didn\'t get that. Did you want to play again? (y/n) ')
